...
def dataset_deconv(
    ...
    devices: Union[str, List[int]] = None,
    check: bool = True,
    ...
    for channel in dataset._selected_channels(channels):
        stack_iter = dataset[channel]

        aprint(f"Slicing with: {stack_iter.slicing}")

        out_shape = tuple(int(round(u * v)) for u, v in zip(stack_iter.shape, (1,) + scaling))
        dtype = np.float16 if method == "admm" else stack_iter.dtype
        ...
        def process(i):
            try:
                with asection(f"Deconvolving time point for time point {i}/{len(stack_iter)}"):
                    with asection(f"Loading channel: {channel}"):
                        tp_array = np.asarray(stack_iter[i])
                    ...
                    with asection(
                        f"Saving deconvolved stack for time point {i}, shape:{tp_array.shape}, dtype:{stack_iter.dtype}"
                    ):
                        dest_dataset.write_stack(channel=channel, time_point=i, stack_array=tp_array)

                    aprint(f"Done processing time point: {i}/{len(stack_iter)} .")
