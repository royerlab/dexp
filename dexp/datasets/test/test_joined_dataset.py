...
def test_joined_dataset():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(3):
            zdataset = ZDataset(path=join(tmpdir, f"test{i}.zarr"), mode="w", store="dir")
        ...
        for channel in joined_dataset.channels():
            for dataset in dataset_list:
                assert joined_dataset.nb_timepoints(channel) == sum(
                    dataset.nb_timepoints(channel) for dataset in dataset_list
                )
