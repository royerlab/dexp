...
def _test_model_io_to_json():
    ...
    model_list = list(
        TranslationRegistrationModel(shift_vector=[-1 * (i / 100.0), -5 + 0.1 * i, 13 - 0.2 * i], confidence=0.6)
        for i in range(0, 10)
    )
    model_list += list(
        WarpRegistrationModel(
...
def _test_model_io_to_file():
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        model_list_1 = list(
            TranslationRegistrationModel(shift_vector=[-1, i, 2 * i], confidence=0.6 - 0.01 * i) for i in range(3)
        )
        model_list_2 = list(
            WarpRegistrationModel(vector_field=vector_field * (1 + 0.1 * i), confidence=confidence / (i + 1e-8))
            for i in range(3)
        )
