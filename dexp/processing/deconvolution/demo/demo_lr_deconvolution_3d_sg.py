...
def _demo_lr_deconvolution(backend, length_xy=256):
    ...
    with timeit("deconvolved"):
        deconvolved = lucy_richardson_deconvolution(backend, blurry, psf, num_iterations=iterations, padding=17, max_correction=max_correction)

    with timeit("lucy_richardson_deconvolution (scatter-gather)"):
        def f(_image):
            return lucy_richardson_deconvolution(backend, _image, psf, num_iterations=iterations, normalise_input=False, padding=17, max_correction=max_correction)

        deconvolved_sg = scatter_gather_i2i(backend, f, blurry, chunks=length_xy // 2, margins=17, normalise=True, to_numpy=True)
